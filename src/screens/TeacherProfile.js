...
import { teacherProfileSchema } from '../schema/formSchema';
import {
  resetAuthReducer,
  editTeacherProfile,
  getUserProfile,
...
export default function TeacherProfile() {
  ...
  const {
    ...
    isErrorEditTeacherProfile,
    isSuccessEditTeacherProfile,
    isLoadingEditTeacherProfile,
  ...
  const formSubmitHandler = (data) => {
    ...
    // }
    dispatch(editTeacherProfile(formData));
  };
  ...
  useEffect(() => {
    if (isErrorEditTeacherProfile) {
      toast.error(messageEditTeacherProfile);
      setShowModal(true);
    }

    if (isSuccessEditTeacherProfile) {
      toast.success(messageEditTeacherProfile);
      setShowModal(false);
      reset();
    }
    if (messageEditTeacherProfile) {
      dispatch(getUserProfile(user._id));
    }
